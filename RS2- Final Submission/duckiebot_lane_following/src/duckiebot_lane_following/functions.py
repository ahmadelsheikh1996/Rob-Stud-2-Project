...
def average_slope_intercept(line_segments):
    ...
    for line_segment in line_segments:
        ...
        elif slope > -0.1 and slope < 0.1 and u1 > left_region_boundary and u2 > left_region_boundary and u1 < right_region_boundary and u2 < right_region_boundary:
             # horizontal lane
             horiz_fit.append((slope, intercept))


    left_fit_average = np.average(left_fit, axis=0)
    right_fit_average = np.average(right_fit, axis=0)
    horiz_fit_average = np.average(horiz_fit, axis=0)
    ...
    # horizontal
    if len(horiz_fit) > 0:
        horiz_lane = line_slope_intercept2points(horiz_fit_average)
